...
```json
{
  ...
  "layers": [
    ...
    { "id": "sub", "type": "subtitle", "name": "Sub", "visible": true, "locked": false, "frozen": false,
      ...
      "box": { "x": 190, "y": 470, "width": 480, "height": 220, "anchor": "topLeft" },
      "textAlign": "center", "verticalAlign": "middle", "wrap": "pixelWidth", "stripEmoji": true, "upper": false,
      "styleAssignment": { "mode": "fixed", "presets": ["gold-serif"] },
      "mirrorWithSide": true }
  ],
  "stylePresets": [
    { "id": "gold-serif", "font": "Georgia", "size": 55, "bold": true, "color": "#F5A623",
      "outlineColor": "#000000", "outline": 2.5, "shadow": 1.0, "lineSpacing": 0 },
    { "id": "white-slab", "font": "Rockwell Condensed", "size": 55, "bold": true, "color": "#FFFFFF",
      "outlineColor": "#000000", "outline": 2.5, "shadow": 0.0, "lineSpacing": 0 }
  ]
}
```
...
- Font chỉ lưu **tên họ font** (family name). Font phải cài trong Windows; ô "file font" là tùy chọn nâng cao (v1 có thể bỏ).
...
- Đọc SRT: thử mã hóa UTF-8 (có/không BOM) -> UTF-16 -> CP1258 -> Latin-1. Chuẩn hóa xuống dòng. Bỏ thẻ `<i>`, `{...}`. Gộp các dòng của 1 cue thành 1 câu rồi **tự ngắt dòng**.
- **Ngắt dòng theo bề rộng pixel của hộp sub** (`box.width`): đo bằng font thật (SkiaSharp/WPF) với hệ số an toàn 0,97, chèn `\N`. `WrapStyle: 2` trong ASS để libass không tự ngắt thêm. Không đếm ký tự.
- Vị trí: `{\an<N>\pos(x,y)}` với N từ `textAlign` + `verticalAlign` của hộp, (x, y) là điểm neo tương ứng trong hộp.
...
| E030 | Error* | Driver không có sub tương ứng (* Warning nếu template cho phép render không sub) |
...
| W070 | Warning | Font trong preset chưa cài |
...
- Layer Sub hiển thị là **hộp chữ** kéo/co giãn được; chiều rộng hộp quyết định ngắt dòng (§7). Preset style chọn ở bảng thuộc tính (font, cỡ, màu, viền, bóng) và xem ngay bằng chữ mẫu.
...
**Tab Hàng đợi**: §10. **Tab Cài đặt**: đường dẫn ffmpeg/ffprobe (tự dò PATH + kiểm tra phiên bản), encoder (Auto/NVENC/x264), chất lượng, thư mục cache, số job song song, ngôn ngữ.
...
6. Người dùng còn phải chốt: font thật của 2 preset (vàng serif, trắng slab), avatar có luôn ở bên phải hay xen kẽ, và ảnh 2 có wave hay không.
