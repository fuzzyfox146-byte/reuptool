...
## Current state
- Milestone: **M7 improved v5 + UI interaction rewrite** (Version 1.0.2). All tests passing!
- Latest: Rewrote `DesignCanvasControl` interaction (unified drag+resize state machine, click-to-select updates property panel, corner resize handles, live 2-way sync canvas↔panel, selection highlight). Fixed lag from conflicting event handlers. Bumped to 1.0.2. All 74 tests pass, build OK.
